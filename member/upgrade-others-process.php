...
if($_SESSION['mid'])
{
$loguserid=getMember($conn,$_SESSION['mid'],'userid');
...
$sqlcp="SELECT * FROM `ee_member_package` WHERE `userid`='".$userid."' AND `package`='".trim($_REQUEST['package'])."'";
$rescp=query($conn,$sqlcp);
$numcp=numrows($rescp);
if($numcp<1)
{
//------------------------------------//
...
if($numc<1)
{
$sql="INSERT INTO `ee_member_upgrade` (`userid`,`package`,`pvalue`,`epin`,`upgradeby`,`date`) VALUES('".$userid."','".$package."','".$pvalue."','".trim($_REQUEST['epin'])."','".$loguserid."','".date('Y-m-d')."')";
$res=query($conn,$sql);
...
redirect('upgrade-others.php?case=check&s=1');
}else{
redirect('upgrade-others.php?case=check&e=2');
}
//-----------------------------------------------//
}else{
redirect('upgrade-others.php?case=check&e=4');
}

}else{
redirect('upgrade-others.php?case=check&e=3');
}
